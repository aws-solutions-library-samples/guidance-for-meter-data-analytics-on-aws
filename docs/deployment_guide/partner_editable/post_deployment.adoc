...
== Input Adapter
The solution's input adapter loads meter reads from an external source (such as HES or FTP) and prepares them for the staging area. This section describes the default input adapter configuration and explains how to edit the adapter to connect to different systems and formats.
